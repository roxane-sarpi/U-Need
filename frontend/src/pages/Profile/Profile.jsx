...
const TABS = [
  { key: 'ads',      label: 'Mes annonces',        count: 4 },
  { key: 'liked',    label: 'Annonces likées',      count: 5 },
  { key: 'services', label: 'Services rendus',      count: 5 },
  { key: 'history',  label: 'Historique de points' },
  { key: 'info',     label: 'Infos personnelles' },
]

const ADS = [
  { id: 1, category: 'BRICOLAGE',    status: 'EN COURS',  pts: 3, title: 'Réparation robinet salle de bain',        desc: 'Fuite au niveau du joint, besoin d\'aide rapidement' },
  { id: 2, category: 'JARDINAGE',    status: 'ACTIVE',    pts: 2, title: 'Taille de haie et débroussaillage',        desc: 'Jardin de 80m², haie de 15m à tailler côté rue' },
  { id: 3, category: 'INFORMATIQUE', status: 'TERMINÉ',   pts: 4, title: 'Installation et configuration PC',         desc: 'Mise en place d\'un poste de travail sous Windows 11' },
  { id: 4, category: 'TRANSPORT',    status: 'BROUILLON', pts: 1, title: 'Déménagement canapé 2 places',             desc: 'Du 3e étage sans ascenseur, environ 2 km de distance' },
]

const LIKED_ADS = [
  { id: 1, category: 'CUISINE',      author: 'Marie D.',    pts: 3, title: 'Cours de pâtes fraîches maison',          desc: '2h de cours pour apprendre la cuisine italienne' },
  { id: 2, category: 'SPORT',        author: 'Théo R.',     pts: 2, title: 'Séance de coaching running débutant',     desc: 'Sortie d\'1h adaptée aux débutants, parc de la Tête d\'Or' },
  { id: 3, category: 'BRICOLAGE',    author: 'Paul M.',     pts: 4, title: 'Pose de carrelage salle de bain',         desc: 'Surface de 6m², matériaux non fournis' },
  { id: 4, category: 'INFORMATIQUE', author: 'Clara S.',    pts: 2, title: 'Aide configuration réseau Wi-Fi',         desc: 'Box Freebox, problème de connexion intermittente' },
  { id: 5, category: 'JARDINAGE',    author: 'Antoine B.',  pts: 3, title: 'Plantation et entretien potager',         desc: 'Conseils et aide pour démarrer un potager de balcon' },
]

const SERVICES = [
  { id: 1, category: 'BRICOLAGE',    date: '12 avr. 2026', title: 'Montage meubles IKEA',                  for: 'Sophie L.',   pts: 2, rating: 5 },
  { id: 2, category: 'INFORMATIQUE', date: '28 mars 2026', title: 'Récupération données disque dur',       for: 'Marc T.',     pts: 4, rating: 5 },
  { id: 3, category: 'TRANSPORT',    date: '15 mars 2026', title: 'Aide au déménagement studio',           for: 'Julie F.',    pts: 3, rating: 4 },
  { id: 4, category: 'JARDINAGE',    date: '2 mars 2026',  title: 'Tonte pelouse et ramassage feuilles',   for: 'Pierre C.',   pts: 2, rating: 5 },
  { id: 5, category: 'CUISINE',      date: '18 fév. 2026', title: 'Préparation repas pour 10 personnes',  for: 'Emma R.',     pts: 5, rating: 4 },
]
...
function IconBtn({ children }) {
  return (
    <button className="w-8 h-8 flex items-center justify-center rounded-lg border border-gray-200 text-gray-400 hover:text-gray-600 transition-colors">
      {children}
    </button>
  )
}
...
function AdRow({ category, status, pts, title, desc }) {
  ...
  return (
    <div className="flex items-center gap-4 border border-gray-100 rounded-xl p-3">
      <div className="w-16 h-16 rounded-lg shrink-0 flex items-center justify-center text-xs text-gray-400 font-medium" style={HATCH_BG}>
        IMAGE
      </div>
      ...
      <div className="flex items-center gap-2 shrink-0">
        <span className="text-xs font-bold rounded-lg px-2 py-1" style={{ backgroundColor: 'var(--color-primary-soft)', color: 'var(--color-primary)' }}>
          {pts} {pts > 1 ? 'PTS' : 'PT'}
        </span>
...
function LikedRow({ category, author, pts, title, desc }) {
  return (
    <div className="flex items-center gap-4 border border-gray-100 rounded-xl p-3">
      <div className="w-16 h-16 rounded-lg shrink-0 flex items-center justify-center text-xs text-gray-400 font-medium" style={HATCH_BG}>
        IMAGE
...
function ServiceRow({ category, date, title, for: forUser, pts, rating }) {
  return (
    <div className="flex items-center gap-4 border border-gray-100 rounded-xl p-3">
      <div className="w-16 h-16 rounded-lg shrink-0 flex items-center justify-center text-xs text-gray-400 font-medium" style={HATCH_BG}>
        IMAGE
      ...
      <div className="shrink-0">
        <span className="text-xs font-bold rounded-lg px-2 py-1 flex items-center gap-1" style={{ backgroundColor: '#DCFCE7', color: '#16A34A' }}>
          <CheckCircle size={11} />
          {pts} {pts > 1 ? 'PTS' : 'PT'}
        </span>
      </div>
  ...
  return (
    <div className="flex items-center justify-between py-3 border-b border-gray-50 last:border-0">
      <div className="flex items-center gap-3">
        <div className="w-8 h-8 rounded-full flex items-center justify-center shrink-0" style={{ backgroundColor: positive ? '#DCFCE7' : '#FEE2E2' }}>
          {positive
      ...
      <span className="text-sm font-bold shrink-0" style={{ color: positive ? '#16A34A' : '#DC2626' }}>
        {positive ? '+' : ''}{delta} pts
      </span>
    </div>
  )
    ...
    <div className="min-h-screen" style={{ backgroundColor: 'var(--color-canvas)' }}>
      <div className="max-w-4xl mx-auto px-6 py-8 flex flex-col gap-4">
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6 flex items-start gap-6">
          <div className="w-20 h-20 rounded-full bg-gray-200 shrink-0 flex items-center justify-center text-2xl font-bold text-gray-400">
            LM
          </div>
          ...
          {/* Solde actuel */}
          <div className="rounded-xl px-6 py-4 text-center shrink-0 min-w-30" style={{ backgroundColor: 'var(--color-primary-soft)' }}>
            <p className="text-xs uppercase tracking-widest text-gray-500 mb-1">Solde actuel</p>
            <p className="text-5xl font-black leading-none mb-1" style={{ color: 'var(--color-ink)' }}>17</p>

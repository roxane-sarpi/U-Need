				...
				<div className="mt-4 space-y-3">
					{categoryOptions.map((category) => (
						<label key={category.key} className="flex cursor-pointer items-center gap-3 text-sm text-ink/80">
							...
							<span className="text-xs text-ink/40">{countsByCategory[category.key] ?? 0}</span>
						</label>
					))}
			...
			<div className="mb-6">
				...
				<div className="mt-4">
					<label className="flex items-center gap-2 rounded-xl border border-ink/15 bg-ink/5 px-3 py-2 text-sm text-ink/45 shadow-inner">
						<MapPin size={14} />
						<input type="text" value={postalCode} onChange={(event) => onPostalCodeChange(event.target.value)} placeholder="Code postal" className="w-full bg-transparent text-sm text-ink outline-none placeholder:text-ink/35" />
					</label>
				</div>
			</div>
				...
				<div className="mt-4 space-y-3 text-sm text-ink/70">
					...
					<div className="flex flex-wrap gap-2 text-xs text-ink/55">
						{[5, 10, 25, 50].map((value) => (
							<button key={value} type="button" onClick={() => onRadiusChange(value)} className={`rounded-full border px-3 py-1 transition-colors ${radius === value ? 'border-primary bg-primary text-white' : 'border-ink/10 bg-white hover:border-primary/40'}`}>
								{value} km
							</button>
						))}
					</div>
